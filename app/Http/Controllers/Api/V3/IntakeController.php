...
{
    /** Reason stamped on stage transitions generated by the manual intake tooling. */
    ...
    public function complete(Request $request, string $ref): JsonResponse
    {
        ...
        DB::transaction(function () use ($request, $registration, $shareholdersMode, &$applied): void {
            ...
            $applied['stage'] = $reset !== ''
                ...
                : $this->advanceStage($registration, (string) $request->input('advance_to_stage', ''));
        });
    ...
    /**
     * Undo an intake-generated stage advance: reset the stage and remove those transitions.
     *
     * Corrects an expediente advanced by mistake (e.g. before the notary confirmed the acta).
     * Only deletes stage transitions this tooling created (matched by its reason) and only
     * when the target is actually behind the current stage — it never touches transitions
     * performed by real users, and never moves an expediente forward.
     *
     * @param  Registration  $registration  The expediente.
     * @param  string  $target  The stage value to fall back to.
     * @return array<string, mixed> What was reset (or a skip note).
     */
    private function resetStage(Registration $registration, string $target): array
    {
        $targetStage = RegistrationStageEnum::tryFrom($target);
        if ($targetStage === null) {
            return ['skipped' => 'unknown_stage', 'given' => $target];
        ...
        if ($currentIndex === false || $targetIndex === false || $targetIndex >= $currentIndex) {
            return ['skipped' => 'not_behind_current', 'current' => $registration->getRawOriginal('stage')];
        }

        // Remove only the intake-generated hops above the target; keep any real-user history.
        $deleted = $registration->stageTransitions()
            ->where('reason', self::INTAKE_ADVANCE_REASON)
            ->delete();

        $registration->update(['stage' => $targetStage->value]);

        return ['reset_to' => $targetStage->value, 'deleted_transitions' => $deleted];
    }
